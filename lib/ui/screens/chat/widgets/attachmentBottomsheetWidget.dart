...
class _AttachmentDialogWidgetState extends State<AttachmentDialogWidget> {
  ...
  onFilesPicked() async {
    ...
    try {
      ...
      List<String> selectedFilePaths = List.from(selectedFiles.map((element) {
      ...
      for (int i = 0; i < tempPaths.length; i++) {
        if (!selectedFilePaths.contains(tempPaths[i].fileName)) {
          ...
          } else if ((await File(tempPaths[i].fileUrl).length()) >= maxFileSizeInBytesCanBeSent) {
      ...
      if (totalNumberOfFilesRemovedBecauseOfMaxNumberLimit != 0 ||
        ...
        if (context.mounted) {
          UiUtils.showMessage(
              context,
              "${totalNumberOfFilesRemovedBecauseOfMaxNumberLimit != 0 ? "${"maxAllowedFilesAre".translate(context: context)} $maxFilesOrImagesInOneMessage, ${"removed".translate(context: context)} $totalNumberOfFilesRemovedBecauseOfMaxNumberLimit ${"extra".translate(context: context)}." : ""} ${totalNumberOfFilesRemovedBecauseOfMaxSizeLimit != 0 ? "${"removed".translate(context: context)} $totalNumberOfFilesRemovedBecauseOfMaxSizeLimit ${"filesAsTheyExceededTheLimitOf".translate(context: context)}" : ""}",
              MessageType.error);
        }
      }
                              ...
                              children: [
                                Center(
                                  child: CustomContainer(
                                    ...
                                    height: 50,
                                    color:
                                        Theme.of(context).colorScheme.accentColor.withOpacity(0.3),
